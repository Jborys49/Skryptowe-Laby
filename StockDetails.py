...
class StockDetails(ctk.CTkFrame):
    ...
    def __init__(self, parent:ctk.CTkFrame,user:User,stock:str):
        try:
            #asyncio.run(self.setup(stock, user.get_date()))
            super().__init__(parent)
            self.user=user
            self.stock=stock

            #self.stock_records, self.volume = self.setup(stock, user.get_date())
            #Event.fetch_and_save_stock_data(stock, user.get_date())
            self.stock_records, self.volume = Event.fetch_and_save_stock_data(stock, user.get_date())
            if not os.path.isfile('Stock_Graphs/'+self.stock+"1w.png"):
            ...
            text1 = ctk.CTkLabel(right_frame, text=self.get_name(self.stock),font=("Helvetica",14))
            ...
            button1 = ctk.CTkButton(right_frame, text="1 Week",command=lambda:self.embed_graph(left_frame,self.weekplot))
            button1.pack(pady=5,fill=ctk.BOTH, expand=True)

            button2 = ctk.CTkButton(right_frame, text="1 Month",command=lambda:self.embed_graph(left_frame,self.monthplot))
            button2.pack(pady=5,fill=ctk.BOTH, expand=True)

            button3 = ctk.CTkButton(right_frame, text="2 Months",command=lambda:self.embed_graph(left_frame,self.month2plot))
            button3.pack(pady=5,fill=ctk.BOTH, expand=True)

            button4 = ctk.CTkButton(right_frame, text="BUY",command=lambda:self.buy_stock(self.user,self.stock,Event.get_price(self.stock,self.user.get_date(),0)))
            button4.pack(pady=5,fill=ctk.BOTH, expand=True)
        except Exception as e:
            print(e)

    def buy_stock(self,user: User, stock: str, price: Decimal):
        '''InvokeTransaction to buy shares of stock'''
        seller = Transaction(user, stock, price, 0)
        seller.mainloop()

    def embed_graph(self,frame: ctk.CTkFrame, image: ImageTk.PhotoImage):
