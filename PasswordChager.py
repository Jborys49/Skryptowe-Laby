...
class PasswordChager(ctk.CTk):
    def __init__(self,user):
        ...
        execute=ctk.CTkButton(main_frame, text="Change",command=lambda: self.changePassword(passentry1.get(),passentry2.get(),self.user,self))
        execute.grid(row=2,column=0,rowspan=2,columnspan=1)

    def changePassword(self,password1: str, password2: str, user: User, master: tk.Tk) -> None:
