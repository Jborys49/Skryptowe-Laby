...
class Transaction(ctk.CTk):
    ...
    def __init__(self,user:User,stock:str,price:Decimal,type:int,sell_frame=None,label=None):
        ...
        if type==0:
            ...
            determiner.pack()
            commence=ctk.CTkButton(self,text="BUY",command=lambda:self.buy(self,self.user,self.stock,self.price,determiner.get()))
            commence.pack()
        else:
            ...
            determiner = ctk.CTkEntry(self)
            determiner.pack()
            commence = ctk.CTkButton(self, text="SELL",command=lambda:self.sell(self,self.user,self.stock,self.price,determiner.get(),sell_frame,label))
            commence.pack()

    def buy(self,master: ctk.CTk, user: User, name: str, price: Decimal, nr_of_shares: str):
    ...
    def sell(self,master: ctk.CTk, user: User, stock: str, price: Decimal, nr_of_shares: str, sell_frame: ctk.CTkFrame,
             label: ctk.CTkLabel):
